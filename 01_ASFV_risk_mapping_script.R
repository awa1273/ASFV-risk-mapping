...
par(mfrow=c(3,7), oma=c(0,0,0,0), mar=c(2,1.2,0.5,0.5), lwd=0.2, col="gray30"); I = 0
...
for (i in 1:19) { # to get a plot per environmental variable in a specific order
  ...
  I = I+1; title(xlab=envVariableTitles[I], cex.lab=0.9, mgp=c(1.0,0,0), col.lab="gray30")
  box(lwd=0.2, col="gray30")
}

vars_to_plot <- c("relative_humidity_winter",
...
cols <- rgb(222, 67, 39, 80, maxColorValue = 255)
...
for (v in vars_to_plot) {
  ...
  x_seq <- seq(min(dataframes[[1]][, v], na.rm = TRUE),
               max(dataframes[[1]][, v], na.rm = TRUE),
               length.out = 100)
  ...
  for (j in 1:40) {
    ...
    newdata <- as.data.frame(lapply(df[, envVariableNames], mean, na.rm = TRUE))
    newdata <- newdata[rep(1, 100), ]
    newdata[, v] <- x_seq
    
    pred <- predict(brt_models[[j]], 
                    newdata,
    ...
    if (first) {
      plot(x_seq, pred,
           type = "l", col = cols, lwd = 0.7,
           ylim = c(0, 1),
           xlab = v, ylab = "Suitability")
      first <- FALSE
    } else {
      lines(x_seq, pred, col = cols, lwd = 0.7)
    }
  }
}
